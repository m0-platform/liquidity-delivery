...
# Show usage
usage() {
    echo "Fill an order on the OrderBook"
    echo ""
    ...
    echo "     [--origin-recipient <address>] [--bridge-adapter <address>] \\"
    echo "     [--bridge-adapter-args <bytes>] [--wormhole]"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  # Full fill with default adapter (Hyperlane)"
    echo "  $0 --env dev --chain arbitrum_sepolia \\"
    echo "     --order-id 0x1234... --origin-chain sepolia \\"
    echo "     --amount-out 1000000000000000000"
    echo ""
    echo "  # Fill with Wormhole adapter (auto-fetches quote)"
    echo "  $0 --env prod --chain moca \\"
    echo "     --order-id 0x1234... --origin-chain mainnet \\"
    echo "     --amount-out 10000 --wormhole"
    echo ""
...
main() {
    ...
    if [[ "$use_wormhole" == true ]]; then
        ...
        # Map chain IDs to Wormhole IDs (source = dest chain where fill happens, dest = origin chain where report goes)
    ...
    # Build forge command
    local forge_cmd="forge script script/test/FillOrder.s.sol"
    forge_cmd="$forge_cmd --rpc-url $rpc_alias"
    forge_cmd="$forge_cmd -vvv"
    forge_cmd="$forge_cmd --ignored-error-codes 2424"
    forge_cmd="$forge_cmd --sig 'run(bytes32,string,uint128,bytes32,address,bytes)'"
    forge_cmd="$forge_cmd $order_id '$origin_chain' $amount_out $origin_recipient $bridge_adapter $bridge_adapter_args"
