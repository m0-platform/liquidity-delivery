#!/bin/bash
# Cancel a test order on the OrderBook
...
# Show usage
usage() {
    echo "Cancel a test order on the OrderBook"
    echo ""
    ...
    echo "     [--bridge-adapter-args <bytes>]"
    echo ""
...
main() {
    ...
    log_step "Canceling order on $chain_name [env: $env]"
    ...
    # Build forge command
    local forge_cmd="FOUNDRY_PROFILE=production forge script script/test/CancelOrder.s.sol"
    forge_cmd="$forge_cmd --rpc-url $rpc_alias"
    forge_cmd="$forge_cmd -vvv"
    forge_cmd="$forge_cmd --ignored-error-codes 2424"
    forge_cmd="$forge_cmd --sig 'run(bytes32,string,bytes)'"
    forge_cmd="$forge_cmd $order_id '$origin_chain' $bridge_adapter_args"
